...
public class MenuItemAdapter extends RecyclerView.Adapter<MenuItemAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull MenuItemAdapter.ViewHolder holder, final int position) {
        ...
        // Setting flavours icons (Chilli etc...)
        if (!menuItems.get(position).getFlavours().isEmpty()){
            holder.flavoursRecyclerView.setLayoutManager(new LinearLayoutManager(context, RecyclerView.HORIZONTAL, false));
            holder.flavoursRecyclerView.setHasFixedSize(true);
            FlavoursAdapter flavoursAdapter = new FlavoursAdapter(context, menuItems.get(position).getFlavours());
            holder.flavoursRecyclerView.setAdapter(flavoursAdapter);

            holder.flavoursRecyclerView.setVisibility(View.VISIBLE);
        } else {
            holder.flavoursRecyclerView.setVisibility(View.GONE);
        }
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        RecyclerView flavoursRecyclerView;
        ...
        ViewHolder(@NonNull View itemView) {
            ...
            flavoursRecyclerView = itemView.findViewById(R.id.flavours_recyclerview);
...
class FlavoursAdapter extends RecyclerView.Adapter<FlavoursAdapter.ViewHolder> {

    Context context;
    ArrayList<Flavour> flavours;

    public FlavoursAdapter(Context context, ArrayList<Flavour> flavours) {
        this.context = context;
    ...
    @NonNull
    @Override
    public FlavoursAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.flavour_item, parent, false);
        return new FlavoursAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull FlavoursAdapter.ViewHolder holder, int position) {
        Glide.with(holder.itemView).load(flavours.get(position).getImageUrl()).into(holder.flavourImage);
    }
    ...
    public class ViewHolder extends RecyclerView.ViewHolder {
        ImageView flavourImage;

        public ViewHolder(@NonNull View itemView) {
            super(itemView);
            flavourImage = itemView.findViewById(R.id.flavour_item_image);
        }
    }
}